...
let max_joltage n line =
  ...
  let rec aux acc digits_remaining line =
    ...
    let new_line = if idx + 1 < List.length line then List.slice line (idx + 1) 0 else [] in
    if digits_remaining > 0
    then aux (max :: acc) (digits_remaining - 1) new_line
    else acc
  in
  let result = List.rev (aux [] n int_line) in
  let len = List.length result in
  List.foldi ~init:0 ~f:(fun idx acc elt -> Int.pow 10 (len - idx - 1) * elt + acc) result
